...
```json
{
  ...
  "comment": {
    "id": "{{Комментарий.Идентификатор}}",
    "description": "{{Комментарий.Текст}}",
    "owner": {
      "id": "{{Комментарий.Автор}}",
      "name": "{{Комментарий.Автор}}"
    },
    "json": "{{Комментарий.JSON}}"
  }
}
```

**Примечание:** 
- Для получения полной информации о комментарии рекомендуется использовать `{{Комментарий.JSON}}`, который содержит все данные комментария в формате JSON.
- Доступные переменные для комментариев:
  - `{{Комментарий.Идентификатор}}` - ID комментария
  - `{{Комментарий.Автор}}` - Автор комментария
  - `{{Комментарий.Текст}}` - Текст комментария
  - `{{Комментарий.Ссылка}}` - Ссылка на комментарий
  - `{{Комментарий.Html-Link}}` - HTML-ссылка на комментарий
  - `{{Комментарий.Прикрепленные файлы}}` - Прикрепленные файлы
  - `{{Комментарий.JSON}}` - Полный JSON-объект комментария (рекомендуется)
